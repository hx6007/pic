 ...
 * 装修效果、平面布局管理
 */
class Pack78 extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  constructor(props) {
    super();
    this.state = props.serverPack;
  }
  render() {
    const { products, images, cover, spaces, styles, sites, designer, department, shop, brand, address, area, decorateDate, status } = this.state;
    ...
    return (<Vbox>
      ...
      <Divider style={{ marginTop: 50 }} />
      <InputRow>
        <Button type="primary" onClick={() => onSave({ products, images, cover, spaces, styles, sites, designer, department, shop, brand, address, area, decorateDate, status })}>　　保 存　　</Button>
      </InputRow>
    </Vbox>);
  }
}
