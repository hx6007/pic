 ...
 * 样板间管理
 */
class Pack4 extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  constructor(props) {
    super();
    this.state = props.serverPack;
  }
  render() {
    const { products, images, cover, spaces, styles, sites, designer, department, shop, brand, address, status } = this.state;
    ...
    return (<Vbox>
      ...
      <Divider style={{ marginTop: 50 }} />
      <InputRow>
        <Button type="primary" onClick={() => onSave({ products, images, cover, spaces, styles, sites, designer, department, shop, brand, address, status })}>　　保 存　　</Button>
      </InputRow>
    </Vbox>);
  }
}
